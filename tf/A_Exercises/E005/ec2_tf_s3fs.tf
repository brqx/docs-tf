...
# 
# data "template_file" "ec2_shell_template_simple" {
#  template = file("${local.tf_shell_path}shell/basic_http_server.x")
#}

data "template_file" "ec2_shell_template" {
  template = file("${local.tf_shell_path}shell/e005/basic_http_server_s3fs.x")
